...
class ApplicationController < ActionController::Base
  ...
  after_filter :add_script, :if => :html_response?
...
describe ApplicationController, :focus => true do
  ...
  it "adds the GoSquared tracking code", :focus => true do
   expect(controller).to receive(:add_script)
   get :index
 end

 it "add the GoSquared tracking code", :focus => true do
   expect(controller).not_to receive(:add_script)
   get :new
 end
